...
| [08-UI-and-Settings.md](08-UI-and-Settings.md) | Меню симулятора: `USimulatorMenuWidget`, секції налаштувань, персистентність через `USaveGame`, `AUAVSimulatorPlayerController`, HUD-віджети (телеметрія: швидкість, висота, дросель, тяга, тангаж, крен), секція середовища (зорі, хмари, дощ, вуличні вогні), українська локалізація UI |
...
| [13-Environment-Actors.md](13-Environment-Actors.md) | Об'єкти середовища: `AEnvironmentActorManager`, зони РЕБ (`AEWZoneActor`), вітрові вектори (`AWindActor`, реально впливають на аеродинаміку), дощ (`ARainEffectManager`, нульова зв'язаність з `AAirplane`), нічне небо (процедурні зорі `M_Stars`: параметри, алгоритм, усунення кіл-муару), нічні вуличні вогні (`AStreetLightsManager`: яскравість 0–100, вибір джерела будівель `Custom`/`Cesium` — дані сканерів `UCustomSurroundingsScannerComponent`/`UCesiumSurroundingsScannerComponent`, один Niagara-компонент `NS_StreetLights`), інструмент карти `configurate_env_actors.py`, формат `env_actors.json` |
