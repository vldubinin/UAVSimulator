...
1. **Точка входу гри** — `AUAVSimulatorGameModeBase::BeginPlay` штовхає прапорці
...
UI-назви властивостей, багато коментарів і вся ця документація — українською
(мова команди розробки). Ідентифікатори C++, топіки ZMQ і ключі JSON —
англійською, як у коді.
