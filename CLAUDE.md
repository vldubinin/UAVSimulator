...
### Environment Actors (rain, fog, night street lights, EW zones, wind)
...
- **`ATimeOfDayManager`** (`Actor/TimeOfDayManager.h/.cpp`) — автоматичний плин часу доби: щотіку додає `DeltaTime × TimeSpeed / 3600` год до `ACesiumSunSky::SolarTime` (throttled `UpdateSun()` раз на `UpdateInterval`), розсилає `OnSolarTimeAdvanced`, на який підписаний `UEnvironmentSectionWidget` (оновлює `SpinBoxSolarTime` + місяць/зорі). `TimeSpeed` — у разів від реального (дефолт 60, `0` = стоїть), `SpinBoxTimeSpeed` (`OptionalWidget`), `UEnvironmentSettingsSave::TimeSpeed`. Дату не чіпає.
- **`AFogManager`** (`Actor/FogManager.h/.cpp`) — туман одним параметром `FogIntensity` [0,100] (`SetFogIntensity`; `0` = вимкнено, `100` = максимум, `SpinBoxFogIntensity` `OptionalWidget`, `UEnvironmentSettingsSave::FogIntensity`). Керує штатним `AExponentialHeightFog` (бере наявний у рівні або спавнить), тож туман видно і в бортовій камері; `FogDensity = MaxFogDensity × (I/100)²`. Лениво спавниться `GetFogManager()`.
...
| `Actor/FogManager.h/cpp` | Fog: `FogIntensity` 0–100 drives an `AExponentialHeightFog` (found in level or spawned); `SetFogIntensity()` |
