...
class UavFlyTest:
    ...
    def run(self) -> None:
        ...
        try:
            while True:
                ...
                if self._navigation_enabled:
                    cmd = self._navigate(drone_pose, now, frame)
                    self._send_command(cmd)
                    self._log_command(cmd, drone_pose)
    ...
    def _navigate(self, drone_pose: Optional[Position], now: float, frame: np.ndarray) -> ControlCommand:
        if self._ref_lat is None:
            return ControlCommand(0.0, 0.0, 0.0, CRUISE_THRUST,
                                   "waiting for reference point (no landmarks seen yet)")

        if drone_pose is not None:
            heading_deg = self._heading_estimator.update(drone_pose, self._ref_lat, self._ref_lon, now)
            self._advance_waypoint_if_reached(drone_pose)
        else:
        ...
        return self._autopilot.compute_command(drone_pose, current_target)

    def _current_target(self) -> Optional[Position]:
        return None if self._wp_index >= len(WAYPOINTS) else WAYPOINTS[self._wp_index]

    def _advance_waypoint_if_reached(self, drone_pose: Position) -> None:
